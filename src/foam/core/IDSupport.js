 ...
 * Use when you want a multipart primary key.
 *
 * Ex.
 * <pre>
 *   foam.CLASS({
...
//    ['hidden', true],
    ...
    {
      name: 'getter',
      value: function multiPartGetter() {
    ...
    {
      name: 'setter',
      value: function multiPartSetter(a) {
        var props = this.cls_.ID.props;

        if ( props.length === 1 ) {
          props[0].set(this, a);
        } else {
          foam.assert(props.length === a.length,
              'Multi-part ID is wrong size.');
  ...
  methods: [
    function installInClass(c) {
      this.props = this.propNames.map(function(n) {
        ...
        foam.assert(foam.core.Property.isInstance(prop),
            'Ids property:', c.id + '.' + n, 'is not a Property.');
        return prop;
      });
  ...
  properties: [
    {
      ...
      assertValue: function(value) {
        foam.assert(foam.Array.isInstance(value), '"ids" must be an array.');
