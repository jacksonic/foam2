...
foam.LIB = function LIB(model) {
  ...
  if ( model.methods ) {
    ...
    for ( i = 0 ; i < model.methods.length ; i++ ) {
      ...
      foam.assert(typeof m === 'object' || typeof m === 'function',
                     'Methods must be a map or a function');

      foam.assert(typeof m.name === 'string' && m.name !== '',
                     'Methods must be named with a non-empty string');

      if ( typeof m === 'object' ) {
        foam.assert(typeof m.code  === 'function',
