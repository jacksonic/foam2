...
foam.LIB({
  ...
  methods: [
    function typeCheck(fn, opt_args) {
       ...
       * check.
       * @param {foam.Function} fn The function to decorate. The toString() of the function must be
       *     accurate.
       * @param {foam.Array=} opt_args In many cases the args are already parsed out or available
       *       from Method.args, so can be provided directly.
       * @return {foam.Function} A new function that will throw errors if arguments
        ...
        postSet: function(old, nu) {
          ...
          if ( ! this.args || ! this.args.length ) {
            try {
              ...
              (this.warn || console.warn)(
                'Method: Failed to extract args from method ' +
                this.name + ':\n' + (nu && nu.toString()) + '\n', e);
              //throw e; //TODO: throw?
            }
          }
