...
/**
  Implements declares a class's intent to implement
  an Interface. Since interfaces can have method implementations, they
  can be used to provide mix-ins, which are a safe form of multiple inheritance.
<pre>
  Ex.
  foam.INTERFACE({
    name: 'SalaryI',
...
foam.CLASS({
  ...
  properties: [
    {
      /**
       * Internal details: the "name" of a Implements axiom when installed is
       * "implements_path.to.Interface".
       */
      name: 'name',
      getter: function() { return 'implements_' + this.path; }
    },
    {
      /** The path (foo.bar.MyInterface) to the interface. */
      name: 'path'
    }
  ],
  ...
  methods: [
    function installInClass(cls) {
      ...
      if ( foam.core.Interface && foam.core.Interface.isInstance(m) ) {
        ...
        throw 'Mix-ins are not supported';
      }
